...
**@mozartspa/gqless-cli** has also a complete programmatic API, which allows you to generate the client with custom scripts.
...
```tsx
import { inspectWriteGenerate } from '@mozartspa/gqless-cli';
...
```tsx
import { generate } from '@mozartspa/gqless-cli';
...
```tsx
import { getRemoteSchema } from '@mozartspa/gqless-cli';
